...
Bee.prototype.act = function(step, level) {
...
BigJ.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
if (type == "lava" && this.status == null ||
      type == "lava" && this.status == "bigJUMP" ||
      type == "spike" && this.status == null ||
	  type == "bee" && this.status == null ||
	  type == "bee" && this.status == "bigJUMP" ) {
    this.status = "lost";
...
deathsCounter++;
deathsDisplayDisplay = document.getElementById('deaths').innerHTML='Deaths: ' + deathsCounter;
